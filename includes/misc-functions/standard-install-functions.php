 ...
 * @return void
 */
function leben_theme_bundle_install() {
	...
	$mp_core_options['mp_stacks_theme_bundle_being_installed'] = array( 
		'theme_bundle_slug' => 'leben_theme_bundle',
		'fancy_title' => 'Leben Theme Bundle',
		'support_url' => 'https://mintplugins.com/support/leben-theme-bundle-support/',
		'support_email' => 'support@mintplugins.com',
		'required_theme_dirname' => apply_filters( 'leben_theme_bundle_required_theme_dirname', 'knapstack' ),
 ...
 * @return void
 */
function leben_theme_bundle_admin_init(){
		
	//Set up the auto redirects to install dependant plugins
	global $mp_core_options, $leben_theme_bundle_installation_routine;
...
add_action( 'init', 'leben_theme_bundle_admin_init' );
 ...
 */
function leben_theme_bundle_initialize_default_items_setup( $mp_core_options, $theme_bundle_slug ){
...
add_filter( 'mp_core_parent_plugin_installation_complete_filter', 'leben_theme_bundle_initialize_default_items_setup', 10, 2 );
 ...
 * @return   void
 */
function leben_theme_bundle_switch_theme_workaround(){		
	...
	$fancy_theme_name = apply_filters( 'leben_theme_bundle_fancy_theme_name', NULL );
	...
	if ( $fancy_theme_name == $current_theme_name && strpos( $current_url, 'mp_core_install_plugins_page' ) !== false ){	
		...
		//Send us back to the admin and away from this error page - before the error page has a chance to show its ugly face.
		wp_redirect( admin_url() );
		exit;
	}
}
add_action( 'wp_die_handler', 'leben_theme_bundle_switch_theme_workaround' );
 ...
 * @return void
 */
function leben_theme_bundle_kill_edd_welcome(){
...
add_action( 'plugins_loaded', 'leben_theme_bundle_kill_edd_welcome' );